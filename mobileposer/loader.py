...
class DataLoader:
    def __init__(self, dataset: str='dip', combo: str='lw_rp'):
    ...
    def _get_sequence(self, seq_num):
        ...
        if self.dataset in datasets.amass_datasets:
            file_path = os.path.join(paths.processed_datasets25, 'eval', f'{self.dataset}.pt')
        elif self.dataset == 'totalcapture':
            file_path = os.path.join(paths.processed_totalcapture, 'eval', 'totalcapture.pt')
        elif self.dataset == 'dip':
            file_path = os.path.join(paths.processed_datasets25, 'eval', 'dip_test.pt')
        elif self.dataset == 'imuposer':
            file_path = os.path.join(paths.processed_datasets25, 'eval', 'imuposer_test.pt')
